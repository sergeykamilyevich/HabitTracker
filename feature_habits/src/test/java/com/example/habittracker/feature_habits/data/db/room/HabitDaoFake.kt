...
import com.example.habittracker.feature_habits.data.db.models.HabitWithDoneDbModel
import com.example.habittracker.feature_habits.data.repositories.DbHabitRepositoryImpl.Companion.PRIMARY_KEY_CONSTRAINT_MESSAGE
import com.example.habittracker.feature_habits.data.repositories.DbHabitRepositoryImpl.Companion.UNIQUE_CONSTRAINT_MESSAGE
...
class HabitDaoFake : HabitDao {
    ...
    val habitDoneToInsert: HabitDoneDbModel =
        HabitDoneDbModel(
            id = 0,
            habitId = 1,
            date = 0
    ...
    val habitToInsert: HabitDbModel =
        HabitDbModel(
            id = 0,
            name = "habit name",
...
//    }
    ...
    override fun getList(
        ...
        habitList.forEach { habit ->
            ...
            habitTypeFilter.forEach {
                if (habit.type == HabitType.valueOf(it))
                    filteredByTypeList.add(
                        HabitWithDoneDbModel(
                            habit,
                            habitDonesList
                        )
                    )
        ...
        val filteredList = when (valueOf(orderBy)) {
